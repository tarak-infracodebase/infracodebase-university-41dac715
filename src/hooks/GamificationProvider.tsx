...
import { useGamification, type GamificationHook, BADGES } from "./useGamification";
...
function BadgeToast({ badge, onDone }: { badge: typeof BADGES[number]; onDone: () => void }) {
  ...
  return (
    <div
      className="flex items-center gap-3 rounded-xl border border-crystal-yellow/30 bg-card/90 backdrop-blur-xl px-4 py-3 shadow-lg animate-in slide-in-from-bottom-2 fade-in"
      style={{ pointerEvents: "none" }}
    >
      <div className="h-8 w-8 rounded-lg bg-crystal-yellow/10 flex items-center justify-center shrink-0">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--crystal-yellow))" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
