...
const videoTopics = [
  "All", "Architecture Design", "Security & Governance", "Deep Dives",
];
...
const videos: VideoItem[] = [
  {
    ...
    trackLabel: "Track 2 — Foundations",
...
function getProgress(id: string): number {
  try { return Number(localStorage.getItem(`vid-progress-${id}`) || 0); } catch { return 0; }
}
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-2">
        ...
        <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary text-[10px] font-medium border border-primary/20 inline-block">
          {video.category}
        </span>
        <Link
          to={video.trackPath}
...
function InlinePlayer({ video, onClose }: { video: VideoItem; onClose: () => void }) {
  ...
  return (
    <div className="glass-panel rounded-2xl overflow-hidden border border-border/30 shadow-xl shadow-primary/5">
      ...
      <video
        ref={ref} controls autoPlay preload="metadata" playsInline
      ...
      <div className="p-4 space-y-3">
        ...
        <Link to={video.trackPath} className="text-xs text-primary hover:underline inline-flex items-center gap-1">
          Go to {video.trackLabel} <ArrowRight className="h-3 w-3" />
        </Link>
        <div className="border-t border-border/30 pt-3">
          <Link to="/dashboard" className="text-xs text-primary hover:underline inline-flex items-center gap-1">
            Apply this to your infrastructure <ArrowRight className="h-3 w-3" />
          </Link>
        </div>
      </div>
    </div>
...
const VideoLibrary = () => {
  ...
  const handlePlay = (v: VideoItem) => setActiveVideo(v);
  const handleClose = () => { setActiveVideo(null); forceUpdate(n => n + 1); };
  ...
  // Recommended for current workspace (mock)
  const recommended = videos.filter(v => v.id === "5th-video" || v.id === "applying");
  ...
  return (
    <AppLayout>
      <div className="p-6 lg:p-8 max-w-6xl mx-auto space-y-10">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold mb-1">Infrastructure Sessions</h1>
          <p className="text-sm text-muted-foreground max-w-2xl">
            Architecture decisions, real infrastructure scenarios, and deep dives to deepen your understanding.
          </p>
        </div>

        {/* Active Player */}
        {activeVideo && <InlinePlayer video={activeVideo} onClose={handleClose} />}

        {/* Continue Learning */}
        {continueWatching.length > 0 && !activeVideo && (
          <section>
            <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-4">Continue Learning</h2>
            <div className="space-y-3">
              {continueWatching.map(v => {
                const p = getProgress(v.id);
                return (
                  <div key={v.id} className="glass-panel rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 cursor-pointer hover:border-primary/30 transition-colors" onClick={() => handlePlay(v)}>
        ...
        {!activeVideo && (
          <section>
            <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-4">Recommended for your workspace</h2>
            <div className="grid sm:grid-cols-2 gap-5">
              {recommended.map(v => <VideoCard key={v.id} video={v} onPlay={handlePlay} />)}
            </div>
          </section>
        )}
        ...
        <section>
          <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground mb-4">
            {selectedTopic === "All" ? "All Sessions" : selectedTopic}
          </h2>
          <div className="grid sm:grid-cols-2 gap-5">
            {filtered.map(v => <VideoCard key={v.id} video={v} onPlay={handlePlay} />)}
          </div>
          {filtered.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-12">No sessions match your search.</p>
          )}
        </section>

        <div className="glass-panel rounded-xl p-5 text-center">
          <p className="text-xs text-muted-foreground">
            More infrastructure sessions will be added as new architectures and patterns are introduced.
          </p>
        </div>
      </div>
    </AppLayout>
